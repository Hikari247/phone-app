...
window.addEventListener("DOMContentLoaded",
    function(){
        if(typeof localStorage === "underfined"){
            window.alert("このブラウザはLocal Storage実装されていません。");
            return;
        }else{
            viewStorage();
            saveLocalStorage();
...
function delLocalStorage(){
    ...
    function(e){
        ...
        table2.addEventListener("click", (p) =>{
            if(p.target.classList.contains("trash") === true){
                let tr = p.target.parsentNode.parsentNode;
                tr.parsentNode.deleteRow(tr.sectionRowIndex);
            }
        });
        let w_cnt = 0;
